...
export function Topbar({
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 min-w-0 items-center justify-between gap-3 overflow-hidden border-b bg-background/80 px-4 backdrop-blur xl:px-6">
      <div className="flex min-w-0 items-center gap-3">
        <Button variant="ghost" size="icon" className="shrink-0 lg:hidden" onClick={onMenu} aria-label="Open navigation">
          <Menu className="size-4" />
        </Button>
        <div className="min-w-0">
          <div className="truncate text-sm font-semibold">Agent Room</div>
          <div className="truncate text-xs text-muted-foreground">Real cloud machines, real agents.</div>
        </div>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        {signedIn ? (
          <>
            <div className="hidden items-center gap-2 rounded-full border border-emerald-300/20 bg-emerald-300/[0.06] px-3 py-1.5 text-xs text-emerald-100 md:flex">
              <ShieldCheck className="size-3.5" />
              {planLabel(session)}
            </div>
            <div className="hidden items-center gap-2 rounded-full border px-3 py-1.5 text-xs text-muted-foreground lg:flex">
              <CreditCard className="size-3.5 text-sky-200" />
              {formatCredits(credits)}
            </div>
            <div className="hidden max-w-[220px] truncate rounded-full border px-3 py-1.5 text-xs text-muted-foreground sm:block">
              {email}
            </div>
          ...
          <Link
            href="/login"
            className="inline-flex h-8 items-center justify-center gap-2 rounded-md border bg-transparent px-3 text-sm font-medium text-foreground transition-colors hover:bg-secondary"
          >
