...
## [P0] Root layout missing `<head>`/viewport + theme color — likely FOUC and missing PWA hints
...
## [P0] No global `not-found.tsx` or `error.tsx` — 404/500 use Next default white pages
...
## [P0] `/api/billing/checkout` returns 502 for any unexpected error, masking root cause
...
## [P1] `(app)` layout has no auth guard at the layout level
...
## [P1] `next` query parameter not preserved on signup-from-pricing flow
...
## [P1] `signupAction` returns generic error on every Supabase failure
...
## [P1] Login redirect after success uses `next` without sanity-checking the user has a profile
...
## [P1] `Sidebar.active` highlighting is wrong for `/dashboard` ↔ `/`
...
## [P1] `CheckoutButton` shows checkout error for 1 second then disappears on retry
...
## [P1] `EventSource` for sandbox events has no reconnect logic
...
## [P1] Sandbox iframe lacks `sandbox` attribute and could be CSP-tightened
...
## [P1] Topbar credits poller fires every 30s for every authenticated tab
...
## [P1] `Sign in to use the sandbox agent` error surfaced as "Sign in required" on the client
